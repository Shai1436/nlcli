...
class TestCommandExecutor:
    ...
    def test_execute_interactive_success(self, mock_run):
        ...
        result = self.executor.execute_interactive("python -c 'print(input())'")
        
        assert result['success'] is True
        assert result['return_code'] == 0
        assert result['exit_code'] == 0
        assert result['output'] == "Interactive command completed"
    
    @patch('subprocess.run')
    def test_execute_interactive_failure(self, mock_run):
        """Test interactive command execution failure"""
        mock_process = Mock()
        mock_process.returncode = 1
        mock_run.return_value = mock_process
        
        result = self.executor.execute_interactive("invalid_interactive_command")
    ...
    @patch('subprocess.run')
    def test_execute_interactive_exception(self, mock_run):
        """Test interactive command execution with exception"""
        mock_run.side_effect = OSError("Interactive error")
        
        result = self.executor.execute_interactive("test_command")
        
        assert result['success'] is False
        assert 'Interactive execution error' in result['error']
        assert 'Interactive error' in result['error']
    
    @patch('subprocess.run')
    def test_execute_interactive_windows(self, mock_run):
        """Test interactive execution on Windows"""
        with patch.object(self.executor, 'platform', 'windows'):
            mock_process = Mock()
            mock_process.returncode = 0
            mock_run.return_value = mock_process
    ...
    def test_execute_with_custom_timeout(self, mock_run):
        ...
        result = self.executor.execute("test_command", timeout=60)
    ...
    def test_execute_interactive_with_cwd(self):
        """Test interactive execution with custom working directory"""
        with tempfile.TemporaryDirectory() as tmpdir:
            with patch('subprocess.run') as mock_run:
                mock_process = Mock()
                mock_process.returncode = 0
                ...
                result = self.executor.execute_interactive("test_cmd", cwd=tmpdir)
    ...
    def test_execute_interactive_unix_specific(self):
        """Test interactive execution on Unix systems"""
        with patch.object(self.executor, 'platform', 'linux'):
            with patch('subprocess.run') as mock_run:
                mock_process = Mock()
                mock_process.returncode = 0
                ...
                result = self.executor.execute_interactive("bash")
                
                # Verify Unix execution path
                mock_run.assert_called_once()
                assert result['success'] is True
